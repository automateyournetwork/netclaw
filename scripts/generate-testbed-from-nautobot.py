...
# Legacy SSH algorithms for old Cisco IOS/IOS-XE switches that modern OpenSSH
# refuses by default. Appended to the unicon ssh command. All use '+' so they are
# additive — modern boxes still negotiate modern algos first; these only add
# legacy fallbacks. Coverage:
#   - KexAlgorithms / HostKeyAlgorithms: Catalyst 3850, etc. (SHA1 KEX, ssh-rsa)
#   - Ciphers / MACs: older Catalyst 3560E, etc. that offer ONLY CBC ciphers and
#     hmac-sha1 (KEX would match but cipher negotiation fails without these).
LEGACY_SSH_OPTIONS = (
    "-o KexAlgorithms=+diffie-hellman-group-exchange-sha1,diffie-hellman-group14-sha1 "
    "-o HostKeyAlgorithms=+ssh-rsa -o PubkeyAcceptedAlgorithms=+ssh-rsa "
    "-o Ciphers=+aes256-cbc,aes128-cbc,3des-cbc -o MACs=+hmac-sha1"
)
