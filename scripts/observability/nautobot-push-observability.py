...
OBS_COLLECTOR_IP = os.environ.get("OBS_COLLECTOR_IP", "192.168.3.252")


def _default_observability() -> dict:
    """Observability targets now point to this host's IP on the interconnect
    (192.168.3.252 or whatever OBS_COLLECTOR_IP is set to). The lab devices
    reach the OBS stack (otel, gobmp, etc.) via host routing instead of a
    shared clab-mgmt bridge.
    """
    return {
        "mgmt_vrf": "clab-mgmt",  # VRF name on the *remote* lab devices
        "snmp": {"community": "public", "access": "ro"},
        "syslog": {
            "host": OBS_COLLECTOR_IP,
            "port": 1514,
            "transport": "udp",
...
def ensure_clab_network() -> None:
    """No-op: clab-mgmt bridge no longer exists on this host.
    The lab is remote; devices and Nautobot reach each other via host routing.
    """
    pass
...
async def verify_intended(client: NautobotClient) -> None:
    checks = {
        "PE1": {
            ...
            "logging_vrf_clab_mgmt": f"logging host {OBS_COLLECTOR_IP} vrf clab-mgmt",
            "ip_sla": "ip sla 10",
            "no_bmp": "bmp server",
        },
        "RR1": {
            "bmp_server": "bmp server 1",
            "bmp_host": OBS_COLLECTOR_IP,
            "bmp_activate": "bmp-activate server 1",
            "logging_global": f"logging host {OBS_COLLECTOR_IP} transport udp port 1514",
        },
        "West-Spine01": {
...
async def main() -> int:
    ...
    print(
        ...
        "'http://localhost:8428/api/v1/query?query=interface_status'"
    )
