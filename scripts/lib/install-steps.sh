...
# ── Extension fragments (install-steps.d/) ────────────────────────
# Source all *.sh files in install-steps.d/ (if the directory exists).
# Fragments define component_install_* functions that the installer
# dispatches by id. This lets fork-specific install steps live in their
# own file without editing this file — zero merge conflicts on upstream pull.
_STEPS_D="$(dirname "${BASH_SOURCE[0]}")/install-steps.d"
if [ -d "$_STEPS_D" ]; then
    for _frag in "$_STEPS_D"/*.sh; do
