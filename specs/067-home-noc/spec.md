...
**Independent Test**: `docker compose -f deploy/home/docker-compose.yml up` (minimal) reaches home-api `/healthz` and Prometheus targets up.
...
Installer/setup asks which firewall, SoT (none/Nautobot/NetBox), and wireless vendor (none/UniFi/…); only prompts for selected credentials; writes `home-noc.yaml` + `.env` keys documented in `.env.example`.
...
- **FR-011**: Feature work MUST be tracked in `specs/067-home-noc/tasks.md` with PR-aligned phases.
