...
## Prerequisites
- ContainerLab topology running (`clab-mgmt` Docker network exists)
...
```bash
docker network inspect clab-mgmt >/dev/null 2>&1 && echo "PASS" || echo "FAIL"
```
...
```bash
curl -sf http://192.168.220.201:8428/health && echo "PASS" || echo "FAIL"
```
...
```bash
curl -sf http://192.168.220.202:3100/ready && echo "PASS" || echo "FAIL"
```
...
```bash
curl -sf http://192.168.220.203:3000/api/health | grep -q "ok" && echo "PASS" || echo "FAIL"
```
...
```bash
curl -sf "http://192.168.220.201:8428/api/v1/query?query=interface_status" | python3 -c "import sys,json; r=json.load(sys.stdin); print(f'PASS ({len(r[\"data\"][\"result\"])} series)' if r['data']['result'] else 'FAIL')"
```
...
```bash
curl -sf -u admin:netclaw "http://192.168.220.203:3000/api/search?query=Network" | python3 -c "import sys,json; r=json.load(sys.stdin); print(f'PASS ({len(r)} dashboards)' if r else 'FAIL')"
```
...
Network: clab-mgmt (192.168.220.0/24)
...
## Step 10: Register VMs in Nautobot (Optional)

After the stack is healthy, register the containers as virtual machines in Nautobot for IP tracking and SoT completeness:
...
nautobot_create_vm_interface(virtual_machine="otel-collector", name="eth0", description="clab-mgmt")
nautobot_assign_ip_to_vm(virtual_machine="otel-collector", interface="eth0", address="192.168.220.200/24")
...
nautobot_create_vm_interface(virtual_machine="victoriametrics", name="eth0", description="clab-mgmt")
nautobot_assign_ip_to_vm(virtual_machine="victoriametrics", interface="eth0", address="192.168.220.201/24")

nautobot_create_virtual_machine(name="loki", cluster="Observability", role="Monitoring", comments="Log aggregation")
nautobot_create_vm_interface(virtual_machine="loki", name="eth0", description="clab-mgmt")
nautobot_assign_ip_to_vm(virtual_machine="loki", interface="eth0", address="192.168.220.202/24")

nautobot_create_virtual_machine(name="grafana", cluster="Observability", role="Monitoring", comments="Dashboards + visualization")
nautobot_create_vm_interface(virtual_machine="grafana", name="eth0", description="clab-mgmt")
nautobot_assign_ip_to_vm(virtual_machine="grafana", interface="eth0", address="192.168.220.203/24")
```

**GATE:** All 4 VMs created with primary IPs assigned. Verify:
```
nautobot_get_virtual_machines(cluster="Observability")
```
Expected: 4 VMs with IPs .200-.203.
