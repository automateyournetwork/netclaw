...
Do NOT create a shorter or simplified query. Do NOT add `custom_fields` or `_custom_field_data` — use the `cf_` shorthand fields exactly as shown above. The templates depend on every field listed. If BGP data is missing or field names are wrong, config generation will fail with GraphQL 400 errors.
