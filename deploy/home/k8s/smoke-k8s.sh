#!/usr/bin/env bash
# Smoke-test NetClaw Home K3s kustomize (067 Phase 4 / T042).
...
echo "Summary: $PASS passed, $FAIL failed"
if (( FAIL > 0 )); then
  echo "Tips: kubectl -n $NS get pods; kubectl -n $NS logs deploy/home-api --tail=40"
  exit 1
fi
echo "OK — HUD env:"
