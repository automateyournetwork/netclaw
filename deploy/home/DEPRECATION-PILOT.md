...
| Pilot home-api k8s | `netclaw/ui/home-api/k8s/` | Deploys Guardian Web into pilot NS |

## Product path (in-repo)

| Artifact | Location |
|----------|----------|
| Docker Home | `netclaw/deploy/home/docker-compose.yml` |
| K3s Home | `netclaw/deploy/home/k8s/` |
| home-api source | `netclaw/ui/home-api/` |
| HUD | `netclaw/ui/netclaw-visual/` |
| Install profile | `./scripts/install.sh --profile home` |

## Migration order

1. **Dual-run** — HUD `HOME_API_URL` may still point at pilot Guardian.
2. **Docker or K3s Home** — stand up `netclaw-home` / compose; smoke.
...
`ensure-guardian-claw.py` and install profile `home` are the supported enrollment path; external-stack-only install is legacy dual-run.
