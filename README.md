...
That's it. The installer deploys 125 skills, installs bundled MCP dependencies, and prepares configuration for 54 MCP integrations, then launches a two-phase setup:
...
| 58 | Splunk | [@splunk/mcp-server2](https://www.npmjs.com/package/@splunk/mcp-server2) | npx (Node) | Log management and SIEM — SPL query validation and execution, index discovery and inspection, saved search management and execution (7 tools) |
...
### Log Management Skills (3)

| Skill | What It Does |
|-------|-------------|
| **splunk-search** | SPL query execution via Splunk MCP: validate search syntax before execution, run queries with time range filtering (earliest/latest), output in Markdown tables. Security event hunting, network log correlation, syslog analysis, and performance trend workflows. |
| **splunk-indexes** | Splunk index discovery and configuration: list all available indexes, inspect index properties (datatype, homePath, coldPath), identify which indexes contain network-relevant data. Capacity planning and data organization workflows. |
| **splunk-saved** | Saved search management: list all saved searches in Splunk apps, execute saved searches by name to run pre-built reports. Network health dashboards, compliance reports, and recurring alert queries. |
