...
function resolveEnvRef(value) {
  // Resolve ${VAR_NAME} references against loaded .env files and process.env
  if (typeof value !== 'string') return value;
  return value.replace(/\$\{([^}]+)\}/g, (_, varName) => {
    // Check process.env first, then parse .env files
    if (process.env[varName]) return process.env[varName];
    for (const f of ENV_FILES) {
      const vars = parseOneEnvFile(f);
...
app.post('/api/chat', async (req, res) => {
  ...
  try {
    ...
    if (gwRes.ok) {
      ...
      fromGateway = true;
      // OpenAI-compat usage block (prompt_tokens / completion_tokens) — backs
      // the HUD footer token strip via /api/tokens/summary.
...
server.listen(PORT, HOST, () => {
  console.log(`NetClaw visual API listening on http://${HOST}:${PORT}`);
