...
function getGatewayConfig() {
  try {
    ...
    // FORK PATCH (upstream bug candidate): openclaw.json may store the token as
    // a "${OPENCLAW_GATEWAY_TOKEN}" reference rather than a literal. Passing that
    // string through verbatim yields a 401 from /v1/*, which the HUD then reports
    // as "gateway offline". Resolve it against process.env and the .env files.
    const rawToken = config?.gateway?.auth?.token || '';
    const token = rawToken.replace(/\$\{([^}]+)\}/g, (_, name) => {
      if (process.env[name]) return process.env[name];
      for (const f of ENV_FILES) {
        const v = parseOneEnvFile(f)[name];
        if (v) return v;
      }
      return '';
    });
...
// ── FORK-LOCAL HOOK ─────────────────────────────────────────────────────────
// The only fork modification to this file besides the ${VAR} fix in
// getGatewayConfig(). All fork-specific routes and static serving live in
// server.local.js so upstream pulls cannot conflict with them. Registered here
// (last, before listen) so upstream's own routes always take precedence and the
// SPA fallback ends up at the very bottom of the stack.
// Disable with HUD_LOCAL_EXTENSIONS=0, or by deleting server.local.js.
if (process.env.HUD_LOCAL_EXTENSIONS !== '0') {
  try {
    const local = await import('./server.local.js');
    const info = local.register(app, {
    ...
    console.log(
      ...
      + `frontend: ${info.servesFrontend ? 'dist/' : 'not built'})`,
    );
  } catch (err) {
    if (err?.code === 'ERR_MODULE_NOT_FOUND' && /server\.local\.js/.test(err.message || '')) {
      console.log('[local] server.local.js absent — running as pristine upstream');
    } else {
      // Do not silently degrade: a broken extension must be visible.
      console.error('[local] fork extensions FAILED to load:', err.message);
    }
  }
}
